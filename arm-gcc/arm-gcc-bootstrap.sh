...
cd $BUILD
$SRC/gcc-$VERSION/configure --prefix=/usr --libdir=$HOST_LIBDIR --build=$HOST_ARCH --host=$HOST_ARCH --target=$ARM_ARCH --enable-multilib --disable-shared --enable-languages=c,c++ --disable-nls --disable-libssp --with-dwarf2 --without-headers --with-newlib
